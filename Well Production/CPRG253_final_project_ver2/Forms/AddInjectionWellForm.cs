...
{
    public partial class AddInjectionWellForm : BaseForms.AddWell
    {
        public List<WaterType> wt = Enum.GetValues(typeof(WaterType)).Cast<WaterType>().ToList();
        public List<WellPad> pads;
        public AddInjectionWellForm(ref List<WellPad> pads)
        {
            InitializeComponent();
        ...
        private void ComboBoxLoad(ref List<WellPad> pads)
        {
            ...
            WaterTypeComboBox.DataSource = wt;
        }

        private void AddInjectionWellForm_Load(object sender, EventArgs e)
        {
        ...
        private void AddButton_Click(object sender, EventArgs e)
        {
            ...
            if (WellPadComboBox.SelectedIndex > -1 && IDTextBox.Text.Length > 0 && SpudDateTextBox.Text.Length > 0)
            {
                ...
                if (WellPadComboBox.SelectedIndex > -1 && isInteger == true && isDateTme == true)
                {
                    ...
                    var well = WellFactories.GetFactory("Injection").GetWell(); // getting well from wellFactories
                    well.Id = Convert.ToInt32(IDTextBox.Text);
                    well.SprudDate = DateTime.Parse(SpudDateTextBox.Text);
                    InjWell injWell = well as InjWell; //cast to InjWell type to be able to add water type
                    
                    WaterType wt;
                    Enum.TryParse(WaterTypeComboBox.Text, out wt); //parse string to enum
                    injWell.WaterType = wt;

                    pads[selectedIndex].Wells.Add(injWell); //add well to well list for the selected wellPad
                    MessageBox.Show("Injection Well Added");
